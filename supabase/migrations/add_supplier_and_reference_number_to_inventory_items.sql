...
-- Drop constraint if it exists, then add constraint to ensure supplier is valid if provided
ALTER TABLE inventory_items
DROP CONSTRAINT IF EXISTS check_supplier_values;

ALTER TABLE inventory_items
ADD CONSTRAINT check_supplier_values 
CHECK (supplier IS NULL OR supplier IN ('3S', 'Crea Glass', 'Kromatix'));

-- Add comment to document the columns
COMMENT ON COLUMN inventory_items.supplier IS 'Supplier of the glass item: 3S, Crea Glass or Kromatix';
